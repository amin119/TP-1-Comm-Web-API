...
export class CvService {
  ...
  async createCv(arg0: { name: string; firstname: string; age: number; cin: string; job: string; path: string }, userId: number, skillIds: number[]): Promise<Cv> {
    ...
    // Trouver les compétences par leurs IDs
...
async updatev2(id: number, updateDto, userId: number) {
  ...
  // Step 4: Get the new skills from the skillIds in the DTO
  const newSkills = await this.skillRepository.findByIds(updateDto.skillIds);

  // Step 5: Assign the new skills to the CV
  cv.skills = newSkills;

  // Step 6: Update the other CV fields (optional)
...
async createv2(createCvDto: CreateCvDto, user: any): Promise<Cv> {
  ...
  console.log('User from middleware:', userFound); // 👈 test ici
